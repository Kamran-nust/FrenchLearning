...
export default function KwiziqModule({ onBack, startDay }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (present) {
        const res = await readSaved("kwiziq-progress");
      ...
      setViewDay(
        startDay ? Math.max(1, Math.min(startDay, KWIZIQ_TOTAL)) : Math.min(finalProgress.current_day, KWIZIQ_TOTAL),
      );
      setPhase("day");
    }
    init();
