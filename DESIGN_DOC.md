...
Two separate native rewrites live next to the web app, using the same Supabase backend (same accounts, tiers, saved progress, sign-in function): `android/` (Kotlin + Jetpack Compose) and `ios/` (Swift + SwiftUI, project generated by XcodeGen). The web app in `frontend/` is not touched by them. Done so far: email/username sign-in, sign-up, log out, tier chip, four themes, Home with "Day N complete" progress, a read-only day viewer, and (phase 2) the study flow for Grammar, Kwiziq, TV5MONDE and Writing - mark day complete, streaks, reset - saved in the web app's exact format so progress is shared, with saving switched off if the load failed. (phase 3) the Writing section with AI feedback - editor, word count, auto-save, feedback within the tier allowance, entries shared with the web app. (phase 4) the Anki flashcards with pronunciation - new words French to English, weighted review words (hard words 5x), streak, practice rounds - using the web app's saved progress, hard-words and card-stats formats. (phase 5) Grammar chapter PDFs for premium and super, read inside the app (tier enforced by the function and by the app). (phase 6) direct Kwiziq/TV5MONDE lesson links and extra per-day links for premium and super (free accounts keep Google searches; only https addresses are ever opened). (phase 7) the day-plan PDF download for premium (1 per rolling 24 hours) and super (unlimited), built on the phone and saved by the person; the limit is enforced by the same database functions as the web app. (phase 8) the admin page for super users (user list, search, tier changes, confirmation before making someone super, own row locked; both database functions are super-only). Every feature of the web app now exists in both phone apps. Plan data is exported from `frontend/src/data` by `node tools/export-plan-data.mjs`. The Android project compiles and its unit tests pass; the iPhone project has not been compiled (it needs a Mac with Xcode). See `android/README.md` and `ios/README.md`.

### 9.6 Anki daily word limits and Super's bigger day (web)

- **Limits per day** (words = cards shown in Anki, counting the day's own session, extra practice and Word Bank review cards): Free 30, Premium 200, Super none. Numbers live in `frontend/src/shared/ankiLimits.js`.
- **A day's own session is never bigger than the limit**, so a day can always be completed (a Free day late in the plan, which would be up to 48 cards, is held to 30). New sessions can't be *started* once today's limit is used; the person sees "Today's limit reached" (with "See plans" for Free) and the counter resets at their local midnight. Extra practice only gets what is left of today's allowance. A session already started may finish, so the total can end slightly over the limit.
- **Counter:** each card counts the first time it is shown; going back over a card doesn't count it twice. Free and Premium see "Words today: N of LIMIT". It is saved with the other progress as `anki-daily` (`{ date, seen }`).
- **Super's day** is the total cards needed to complete a day (new words plus reviews). It rises steadily from 25 on day 1 to 50 at the end of the plan (`SUPER_DAY_MIN` / `SUPER_DAY_MAX` in `ankiLimits.js`; change those two numbers to retune it). Reviews fill the rest with earlier days' words and Word Bank words; before there are enough earlier words, the Word Bank fills the gap (Super's starter list has 148 words).
- **Limitation, on purpose for now:** Anki runs on the person's own device, so the daily counter lives there too and a determined person could reset it. This stops normal use only.
- **Later: server-side Anki limits.** To make the limits tamper-proof: keep a per-user daily counter in the database (like `feedback_usage`), have sessions built or approved by a function that checks the tier and the count, and refuse new sessions past the limit. It also lets the limits and Super's day size be changed without an app update. Not built yet.
- **Word Bank** (Premium and Super personal word list feeding the Anki reviews; migrations `0010`/`0011`, `frontend/src/modules/WordBankModule.jsx`) and these Anki limits exist on the web, Android (verified on an emulator) and iPhone (written, not compiled). Same rules, database tables and saved `anki-daily` value on all three.

### 9.7 Forgot password, account deletion and parity across the three apps

- **Forgot password** (web, Android, iPhone): "Forgot password?" on the sign-in screen asks for the email and sends a reset link (Supabase Auth). The message never says whether the account exists. The link opens the web app, which already handles the recovery link ("Set a new password"); the phone apps don't need their own deep link. Before launch, set up a custom email sender (SMTP) in Supabase: the built-in sender is heavily rate-limited.
- **Delete my account** (web, Android, iPhone; not offered to Super accounts): a screen with a warning, a typed "DELETE" and the password. It calls the `delete-account` Edge Function (`supabase/functions/delete-account`), which needs a signed-in caller, refuses Super users (403), re-checks the password, then deletes the auth user; saved progress, tier, Word Bank, profile and usage records go with it because every table references `auth.users` with `on delete cascade`. The public pages Google Play asks for are at `/delete-account` (deletion) and `/privacy` (privacy policy, `frontend/src/screens/PrivacyPolicyPage.jsx`) (`frontend/src/screens/DeleteAccountInfoPage.jsx`): it opens without signing in, and `frontend/vercel.json` makes Vercel serve the app for any address. Put the address people can email for deletion in `frontend/src/shared/siteConfig.js` (`SUPPORT_EMAIL`); until it is set the page tells people to use the store page instead.
- **Plans screen** now exists on all three (web, Android, iPhone). The Plans table lists the Word Bank for Premium; there is deliberately no Super column. Payments are placeholders everywhere (web: the Stripe function answers "not configured"; Android: Google Play Billing; iPhone: Apple In-App Purchase).
- **Left out on purpose:** "Browse by level" exists on the web only.

### 9.8 Security audit (2026-09-20)

A full pass over the database, Edge Functions, web and both mobile apps. The posture was already strong; findings and the one code fix:

- **No leaked secrets.** The whole working tree and 66 commits of history were searched for service-role keys, `sb_secret_`, Google API keys, PEM private keys and service-account JSON: none present. The only keys in the repo (`frontend/.env`, Android `BuildConfig`) are the Supabase *publishable/anon* key and project URL, which are public by design (they ship in every browser bundle and app binary); security rests on RLS and Edge Function auth, not on hiding them. The real secrets live only as Supabase Edge Function secrets on the server.
- **RLS** is enabled on every `public` table. User-owned tables scope every row to `auth.uid() = user_id`; internal tables (`feedback_usage`, `pdf_downloads`, `signin_failures`, `word_bank_starter`) have RLS on with no policies, so only the service role reaches them. Tier changes and the admin user list are gated `super`-only in the database, not just the UI. Verified live: a user sees only their own rows and a cross-user write is refused (403).
- **Every `SECURITY DEFINER` function pins `search_path = public`** (no search-path-injection escalation) and each is revoked from `anon`/`public` except the deliberately public `username_available`.
- **Edge Functions** authenticate before doing work and keep the service-role/Gemini/Google/Stripe keys server-side. `sign-in` rate-limits by IP and name, returns one generic error, and does a dummy sign-in for unknown names so timing doesn't leak. `delete-account` requires a session, refuses `super`, and re-checks the password. Prompt text sent to Gemini is the user's own writing shown only back to them (no cross-user or tool surface), and its output is rendered as plain text.
- **No XSS**: no `dangerouslySetInnerHTML`/`eval`/`innerHTML`; AI feedback is plain text; outbound links use `rel="noopener noreferrer"` and only `https://` links are ever opened (web/Android/iOS). iOS tokens live in the Keychain; Android uses private prefs with `allowBackup=false`.
- **Fix — `app_state` size cap (migration `0012`).** RLS confined writes to a user's own rows but nothing bounded their size, so a signed-in user could store a huge value in one key and run up the database. Added `CHECK (octet_length(value::text) <= 4 MB)` and `CHECK (length(key) <= 64)`. The largest legitimate value today is ~1 KB and a full 301-day plan of writing entries stays well under 4 MB, so real use is unaffected; multi-megabyte blob abuse is rejected. Verified in SQL (1 MB accepted, 5 MB and 80-char key rejected).
