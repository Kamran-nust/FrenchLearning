...
- **Delete my account** (web, Android, iPhone; not offered to Super accounts): a screen with a warning, a typed "DELETE" and the password. It calls the `delete-account` Edge Function (`supabase/functions/delete-account`), which needs a signed-in caller, refuses Super users (403), re-checks the password, then deletes the auth user; saved progress, tier, Word Bank, profile and usage records go with it because every table references `auth.users` with `on delete cascade`. Google Play also asks for a public web page explaining how to request deletion; that page is not built yet.
