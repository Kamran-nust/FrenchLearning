...
- **Jump to a day keeps your place**: open a section from a day page and Back returns to that same day page with that section selected (even if you moved to another day inside the section). Opening a section from Home returns to Home. Prev/Next on the day page keep the selected section.
- **Plans (Free vs Premium)**: "Plans and pricing" on Home (and a "Go Premium" pill for free accounts) opens a page with a Monthly/Yearly choice ($12 / $120, yearly saves $24) and a Free-versus-Premium table. Buying must go through Google Play Billing; it is not connected yet (needs a Play Console subscription product), so "Upgrade to Premium" and "Manage subscription" currently say purchases are not switched on and charge nothing. The connection point is `PlayBilling` in `data/Plans.kt`.
- **Forgot password / Delete my account**: "Forgot password?" on sign-in emails a reset link (the link opens the web app to choose the new password). "Delete my account" at the bottom of Home (not shown to Super accounts) asks for the typed word DELETE and the password, then the server deletes the account and everything saved with it.
- **Word Bank (Premium and Super)**: "Word Bank" on Home is a personal word list. Premium starts with 45 starter words, Super with 148; both can add, edit and delete their own words (Premium up to 500 of their own, Super unlimited) and hide starter words or reset them. The words join the random review cards in Anki from the next session (about a quarter of the review slots, tagged "my word"). Free accounts see it locked. Same database tables and rules as the web app.
- **Anki daily limits**: Free can see 30 Anki words a day, Premium 200, Super unlimited; Free and Premium see "Words today: N of LIMIT". A day is never bigger than the limit, and once today's words are used up no new session starts until tomorrow. Super's day is 25 cards rising to 50 across the plan. The counter is saved as `anki-daily` (shared with the web app) and lives on the device.
...
## Verified
Compiles with Gradle 8.13 / AGP 8.10.1 / Kotlin 2.1.20 (`./gradlew :app:testDebugUnitTest :app:assembleDebug`): build succeeds, 75 unit tests pass, debug APK is about 10 MB. The exact server calls the app makes were also tested against the live backend. The app has been run on an Android 16 emulator: the day-plan PDF was drawn (accents, table, page breaks over several pages checked by eye), downloaded through the real save dialog, and the greyed-out button and refund-on-cancel were confirmed; the admin page was also run there (list, search, tier change, confirm/cancel for making someone super, locked own row). A full walkthrough was also done on the emulator (premium and free test accounts): expired sign-in returns to the sign-in screen, sign-in form validation, all five sections (Grammar with the chapter PDF viewer, Kwiziq/TV5 with direct links, Writing with a real AI feedback request, Anki with pronunciation), marking days complete, progress saved in the web app's format, Home's "Day N complete", themes, restart persistence, the free-account locks and log-out. It found and fixed: an expired sign-in that could not be renewed stayed on Home instead of going back to sign-in, the Get feedback button was drawn solid, the status-bar clock was invisible on the light themes, content sat under the gesture bar, long lesson titles were cut off, and AI feedback showed raw markdown asterisks. Emulator tests: `./gradlew connectedDebugAndroidTest` (needs a running emulator or phone).
